...
fn eval_shallow<T: Clone>(loc: Loc<T>) -> EvalResult<T> {
    match loc {
        Loc(Term::Var(s), c) => match c.take(&s) {
            Option::None => Result::Err(EvalError::ReferenceError(ReferenceError::NotFound(s))),
            Option::Some((_, t_box)) => Result::Ok(Fix::Pro(Loc(*t_box, c)))
        },
        Loc(Term::App(fun_box, arg_box), c) => {
            ...
            match fun {
                ...
                Term::Abs(id, y, term_box) => {
                    Result::Ok(Fix::Pro(Loc(Term::Let(id, Option::Some((y, arg_box)), term_box), c)))
                },
                _ => {
                    Result::Ok(Loc::down(Loc(Term::App(Box::new(fun), arg_box), c)))
                }
            }
        },
        Loc(Term::Let(s, o, t_box), c) => {
            let t: Term<T> = *t_box;
            match t {
                Term::Val(_) => Result::Ok(Fix::Pro(Loc(t, c))),
                Term::Abs(abs_s, abs_y, abs_t_box) => {
                    if s == abs_s {
                        // the abstraction shadows the let,
                        // so it is save to drop it.
                        Result::Ok(Fix::Pro(Loc(Term::Abs(abs_s, abs_y, abs_t_box), c)))
                    } else {
                        // the abstraction does not shadow the let,
                        // so it is save to move it into the abstraction.
                        Result::Ok(Fix::Pro(Loc(Term::Abs(abs_s, abs_y, Box::new(Term::Let(s, o, abs_t_box))), c)))
                    }
                },
                _ => {
                    Result::Ok(Loc::down(Loc(Term::Let(s, o, Box::new(t)), c)))
                }
            }
        }
